...
let HeadingComponent = class HeadingComponent {
    ...
    ngOnInit() {
        this.router.events
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["filter"])(e => e instanceof _angular_router__WEBPACK_IMPORTED_MODULE_2__["NavigationStart"]), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["takeUntil"])(this.destroy$))
            .subscribe(() => this.disableSearch());
        Object(rxjs__WEBPACK_IMPORTED_MODULE_3__["fromEvent"])(document.body, 'click')
            ...
            if (this.contactsOpen) {
                if (this.contactsClick) {
                    this.contactsClick = Math.max(0, this.contactsClick - 1);
                }
                else {
                    setTimeout(() => {
                        if (!this.contactsClick) {
    ...
    search(s) {
        if (!s || s.length < 3) {
            ...
            this.searchTypes = this.results ? Object.keys(this.results).map(t => this.results[t][0].type) : null;
        }
    }
    ngOnDestroy() {
        this.destroy$.next();
...
let HeadingModule = class HeadingModule {
};
...
let LandingCarouselComponent = class LandingCarouselComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let LandingCarouselModule = class LandingCarouselModule {
};
...
let GeneralLayoutComponent = class GeneralLayoutComponent {
    constructor(device) {
        this.device = device;
    }
    ngOnInit() {
...
GeneralLayoutComponent.ctorParameters = () => [
...
let GeneralLayoutModule = class GeneralLayoutModule {
};
...
let EmployeePageComponent = class EmployeePageComponent {
    constructor(route) {
        this.route = route;
    ...
    ngAfterViewInit() {
        this.route.params.subscribe(({ id }) => {
            if (Object(util__WEBPACK_IMPORTED_MODULE_4__["isString"])(id)) {
                const num = Math.min(src_app_configuration_teacher_teachers__WEBPACK_IMPORTED_MODULE_3__["default"].length - 1, Math.max(0, parseInt(id, 10)));
                document.body.scrollTop =
                    this.teacherListRef.nativeElement.children[num].getBoundingClientRect().top +
                        document.body.scrollTop;
            }
        });
    }
};
EmployeePageComponent.ctorParameters = () => [
...
let HealingPageComponent = class HealingPageComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let HealingPageModule = class HealingPageModule {
};
...
let LandingPageComponent = class LandingPageComponent {
    ...
    scroll(reverse) {
        ...
        let c = s => this.scrollBox.nativeElement.children[s].offsetLeft;
        let scrollFrom = c(this.slide);
        let scrollTo = 0;
        if (reverse) {
        ...
        const scrollWidth = this.scrollBox.nativeElement.scrollWidth - this.scrollBox.nativeElement.offsetWidth;
        ...
        if (scrollTo !== scrollFrom) {
            ...
            const scrollSteps = scrollTime / scrollInterval;
            const scrollPerStep = (scrollTo - scrollFrom) / scrollSteps;
            let scrollStep = 0;
            const stop$ = new rxjs__WEBPACK_IMPORTED_MODULE_2__["Subject"]();
            Object(rxjs__WEBPACK_IMPORTED_MODULE_2__["interval"])(scrollInterval)
                .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["takeUntil"])(stop$))
                .subscribe(() => {
                if (++scrollStep === scrollSteps) {
                    this.scrollBox.nativeElement.scrollLeft = scrollTo;
                    stop$.next();
                    stop$.complete();
                    this.scrolling = false;
                }
                else {
                    this.scrollBox.nativeElement.scrollLeft = scrollFrom + (scrollPerStep * scrollStep);
                }
            });
        }
    }
...
let LearningPageComponent = class LearningPageComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let SciencePageComponent = class SciencePageComponent {
    constructor() { }
    ngOnInit() {
    }
};
